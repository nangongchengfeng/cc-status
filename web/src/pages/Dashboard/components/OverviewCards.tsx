...
function SmallMetricCard(props: { title: string; value: string; note: string; change?: string; isPositive?: boolean; compareLabel?: string; secondaryValue?: string; unit?: string; accent?: string; compact?: boolean }) {
  return (
    <article className={[
        props.compact
          ? 'rounded-[20px] border border-white/80 px-3 pt-2 pb-3 shadow-[0_12px_32px_rgba(111,153,200,0.14)] backdrop-blur-xl flex flex-col justify-between'
          : 'rounded-[28px] border border-white/80 px-4 pt-3 pb-4 shadow-[0_18px_48px_rgba(111,153,200,0.14)] backdrop-blur-xl flex flex-col justify-between',
        props.accent || 'bg-white/72',
      ].join(' ')}
    >
      <div>
...
function InfoCard(props: { title: string; value: string; note: string; accent?: string; compact?: boolean; extraCompact?: boolean }) {
  return (
    <article className={[
        props.extraCompact
          ? 'rounded-[20px] border border-white/80 px-3 py-8 shadow-[0_12px_32px_rgba(112,151,194,0.16)] backdrop-blur-xl'
          : props.compact
          ? 'rounded-[20px] border border-white/80 px-3 pt-2 pb-3 shadow-[0_12px_32px_rgba(112,151,194,0.16)] backdrop-blur-xl flex flex-col justify-between'
          : 'rounded-[28px] border border-white/80 px-4 pt-3 pb-4 shadow-[0_16px_40px_rgba(112,151,194,0.16)] backdrop-blur-xl flex flex-col justify-between',
        props.accent || 'bg-white/72',
      ].join(' ')}
    >
